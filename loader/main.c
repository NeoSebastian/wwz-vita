...
int debugPrintf(char *text, ...) {
	...
	SceUID fd = sceIoOpen("ux0:data/wwz_log.txt", SCE_O_WRONLY | SCE_O_CREAT | SCE_O_APPEND, 0777);
	if (fd >= 0) {
...
static int (* WwzApplication__OrigLoadSlot)(void **fadeload,int saveSlot);
static uint64_t (* OS_FileGetDate)(int area, const char *path);

int WwzApplication__OrigContinue(void *this, int a2, int a3, int a4) {
	...
	return WwzApplication__OrigLoadSlot(this,latestSave);
}
...
void patch_game(void) {
	hook_addr(so_symbol(&wwz_mod, "__cxa_guard_acquire"), (uintptr_t)&__cxa_guard_acquire);
	hook_addr(so_symbol(&wwz_mod, "__cxa_guard_release"), (uintptr_t)&__cxa_guard_release);

	hook_addr(so_symbol(&wwz_mod, "_Z24NVThreadGetCurrentJNIEnvv"), (uintptr_t)NVThreadGetCurrentJNIEnv);

	// do not use pthread
	hook_addr(so_symbol(&wwz_mod, "_Z15OS_ThreadLaunchPFjPvES_jPKci16OSThreadPriority"), (uintptr_t)OS_ThreadLaunch);
	hook_addr(so_symbol(&wwz_mod, "_Z13OS_ThreadWaitPv"), (uintptr_t)OS_ThreadWait);

	hook_addr(so_symbol(&wwz_mod, "_Z17OS_ScreenGetWidthv"), (uintptr_t)OS_ScreenGetWidth);
	hook_addr(so_symbol(&wwz_mod, "_Z18OS_ScreenGetHeightv"), (uintptr_t)OS_ScreenGetHeight);

	// TODO: set deviceChip, definedDevice
	hook_addr(so_symbol(&wwz_mod, "_Z20AND_SystemInitializev"), (uintptr_t)ret0);

	// TODO: implement touch here
	hook_addr(so_symbol(&wwz_mod, "_Z13ProcessEventsb"), (uintptr_t)ProcessEvents);

	// no touch sense.
	hook_addr(so_symbol(&wwz_mod, "_ZN10TouchSenseC2Ev"), (uintptr_t)TouchSense__TouchSense);
	hook_addr(so_symbol(&wwz_mod, "_ZN10TouchSense20stopContinuousEffectEv"), (uintptr_t)ret0);
	hook_addr(so_symbol(&wwz_mod, "_ZN10TouchSense14stopAllEffectsEv"), (uintptr_t)ret0);
	hook_addr(so_symbol(&wwz_mod, "_ZN10TouchSense28startContinuousBuiltinEffectEiiii"), (uintptr_t)ret0);
	hook_addr(so_symbol(&wwz_mod, "_ZN10TouchSense25playBuiltinEffectInternalEii"), (uintptr_t)ret0);
	hook_addr(so_symbol(&wwz_mod, "_ZN10TouchSense17playBuiltinEffectEiiii"), (uintptr_t)ret0);

	ZIPFile__EntryCompare = (void *)so_symbol(&wwz_mod, "_ZN7ZIPFile12EntryCompareEPKvS1_");
	hook_addr(so_symbol(&wwz_mod, "_ZN7ZIPFile11SortEntriesEv"), (uintptr_t)ZIPFile__SortEntries);

	hook_addr(so_symbol(&wwz_mod, "_ZN11Application4ExitEv"), (uintptr_t)Application__Exit);

	// load latest save
	FadeLoadScene__Loading = (void *)so_symbol(&wwz_mod, "_ZN13FadeLoadScene7LoadingEv");
	cMemCard__HasSave = (void *)so_symbol(&wwz_mod, "_ZN8cMemCard7HasSaveE11MemCardSlot");
	WwzApplication__OrigLoadSlot = (void *)so_symbol(&wwz_mod, "_ZN16WwzApplication12OrigLoadSlotE11MemCardSlot");
	OS_FileGetDate = (void *)so_symbol(&wwz_mod, "_Z14OS_FileGetDate14OSFileDataAreaPKc");
	hook_addr(so_symbol(&wwz_mod, "_ZN16WwzApplication12OrigContinueEv"), (uintptr_t)WwzApplication__OrigContinue);
}
...
int main(int argc, char *argv[]) {
	...
	if (so_load(&wwz_mod, SO_PATH, LOAD_ADDRESS) < 0)
		fatal_error("Error could not load %s.", SO_PATH);

	stderr_fake = stderr;
	stdin_fake = stdin;
	so_relocate(&wwz_mod);
	so_resolve(&wwz_mod, default_dynlib, sizeof(default_dynlib), 0);

	patch_openal();
	patch_game();
	patch_movie();
	so_flush_caches(&wwz_mod);

	so_initialize(&wwz_mod);
